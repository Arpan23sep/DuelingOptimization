...
#' @param alpha A numeric value representing the strong convexity parameter.
#' @param beta A numeric value representing the smoothness parameter.
#' @param tolerance A numeric value representing the desired accuracy for convergence.
...
#'
#' - **x**: Initial starting point in \eqn{R^d}.
#' - **D**: Initial distance between \eqn{x_1} and the optimal point.
#' - **K_\{\epsilon\}**: Number of phases needed for convergence.
#' - **T_\{\epsilon\}**: Query budget.
#' - **η**: Learning rate (step size) for controlling the update size at each iteration.
#'
#' @examples
#' initial_point <- c(0, 0)
#' alpha <- 0.1
