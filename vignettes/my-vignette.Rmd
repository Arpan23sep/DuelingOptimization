...
comparing(resampling) two points using the provided objective function.
The original function is written in RCPP.
...
real-world applications like recommender systems or customer preference
...
This theorem ensures that our algorithm will converge. Suppose Algorithm
1 is run with $$
...
#nd a vector containing the best function value observed at each iteration
print(optimal_point$optimum)           #printing the optimum value 
```

#### Observation:

Optimal value is 0, as expected. Minimum value of square function is 0.

### Multiple Linear Regression using beta_NGD function

We run our linear regression algorithm on the above explained case and
get.
...
Both the results from the lm function and our algorithm are quite
similar. However, our algorithm achieves this without relying on any
model assumptions.
...
If we observe the array or print the objective function value over
iterations, we can conclude that our beta_NGD_optimum is converging more
rapidly to optimal point.
...
We can see that lm output and our output are pretty close and also the
algorithm is converging pretty first.
...
decreases exponentially across phases (starting with $\epsilon_1 = 1$).
...
As the objective function of simple norm calculation of a vector is
α-Strongly Convex and β-Smooth, so we can use our algorithm for this. We
will run our algorithm on this.
...
We can see the optimal value is close to 0, as expected but this time,
much faster.

#### Effects of Linear Regression as no of features or dimension(d) increases:

We will change our dimension and see effects of our plots. Here is the
plot for $$d=2,3,4,5$$
...
of incorrect comparisons caused by noise. We use our this sign_recovery
function for this.
...
#### Observation: 
It's returning true sign as exapected by doing resampling multiple times.

### Our Extension:

Now, we have made some extensions of our work. Firstly, we will show for
the class of functions which satisfy the famous PL inequality.
...
Here is one example demontrating this case. This function is neither
convex nor concave but satisfies PL inequality.
...
#Caling beta_NGD_optimum function
result <- beta_NGD_optimum(initial_point, D, eigen_max, epsilon=0.1,f_value)
...
From the plot,we can see that it is converging to true optimum pretty
first. This function satisfies the condition of PL inequality.
...
#### Observation:
Loss is decreasing to 0 rapidly. So, also in this general non-optimization case, our algorithm is working.
### Conclusion:
